...
class AssetControl(models.Model):
    ...
    def calculate_values(self):
        for rec  in self :
            controls = self.env['asset.control.line'].search([('asset_control_id','=',self.id)])
            if len(controls)>0:
    ...
    def asset_control(self):
        for rec in self :
            self.env['asset.control.line'].search([('id','!=',False)]).unlink()
            ...
            assets = self.env['account.asset.asset'].search([('name','!=',False)])
            
            for asset in assets:
                moves_lines  = self.env['account.move.line'].search([('account_id','in',[asset.category_id.account_immo_id.id,asset.category_id.account_asset_id.id])])
                for line in moves_lines:
                    if line.move_id.asset_id.id == asset.id:
                        if str(line.account_id.code[0]+line.account_id.code[1]) in ['21','22','23','28']:
                            controls = self.env['asset.control.line'].search([('account_id','=',line.account_id.id),('asset_control_id','=',self.id)])
                            if  asset.date.year == self.fiscal_year.date_end.year :
                                if len(controls)>0:
                                    line_value = 0
                                    asset_value = 0
                                    for control in controls:
