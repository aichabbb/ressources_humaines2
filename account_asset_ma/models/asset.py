...
class AccountAsset(models.Model):
    ...
    first_depriaction_value = fields.Float(string='First depreciation value')
    ...
    def compute_depreciation_board(self):
        ...
        if self.value_residual != 0.0 and not self.vna_move_id:
            ...
            amount_to_depr = self.value_residual - self.first_depriaction_value
            ...
            for x in range(len(posted_depreciation_line_ids), undone_dotation_number):
                ...
                if float_is_zero(amount, precision_rounding=self.currency_id.rounding):
                    continue
                if x==0:
                    residual_amount -= self.first_depriaction_value
                    vals = {
                        'amount': self.first_depriaction_value,
                        'asset_id': self.id,
                        'sequence': sequence,
