...
class AccountAssetCategory(models.Model):
    ...
    name = fields.Char(required=True, index=True, string="Nom de categorie")
    account_analytic_id = fields.Many2one('account.analytic.account', string='Compte analytique')
    account_asset_id = fields.Many2one('account.account', string='Compte d\'immobilisation', required=True, domain=[('internal_type','=','other'), ('deprecated', '=', False)], help="Account used to record the purchase of the asset at its original price.")
    account_depreciation_id = fields.Many2one('account.account', string='Écritures d\'amortissement : compte d\'immobilisations', required=True, domain=[('internal_type','=','other'), ('deprecated', '=', False)], help="Account used in the depreciation entries, to decrease the asset value.")
    account_depreciation_expense_id = fields.Many2one('account.account', string='Écritures d\'amortissement : compte de charges', required=True, domain=[('internal_type','=','other'), ('deprecated', '=', False)], oldname='account_income_recognition_id', help="Account used in the periodical entries, to record a part of the asset as expense.")
    journal_id = fields.Many2one('account.journal', string='Journal', required=True)
    company_id = fields.Many2one('res.company', string='Société', required=True, default=lambda self: self.env['res.company']._company_default_get('account.asset.category'))
    method = fields.Selection([('linear', 'Linear'), ('degressive', 'Degressive')], string='Méthode de calcul', required=True, default='linear',
        help="Choose the method to use to compute the amount of depreciation lines.\n"
            "  * Linear: Calculated on basis of: Gross Value / Number of Depreciations\n"
            "  * Degressive: Calculated on basis of: Residual Value * Degressive Factor")
    method_number = fields.Integer(string='Nombre d\'amortissements', default=5, help="The number of depreciations needed to depreciate your asset")
    method_period = fields.Integer(string='Durée de la période', default=1, help="State here the time between 2 depreciations, in months", required=True)
    method_progress_factor = fields.Float('Facteur dégressif', default=0.3)
...
class AccountAssetAsset(models.Model):
    ...
    name = fields.Char(string='Nom', required=True, readonly=True, states={'draft': [('readonly', False)]})
    code = fields.Char(string='Référence', size=32, readonly=True, states={'draft': [('readonly', False)]})
    value = fields.Float(string='Valeur brute', required=True, readonly=True, digits=0, states={'draft': [('readonly', False)]}, oldname='purchase_value')
    currency_id = fields.Many2one('res.currency', string='Monnaie', required=True, readonly=True, states={'draft': [('readonly', False)]},
        default=lambda self: self.env.user.company_id.currency_id.id)
    company_id = fields.Many2one('res.company', string='Société', required=True, readonly=True, states={'draft': [('readonly', False)]},
        default=lambda self: self.env['res.company']._company_default_get('account.asset.asset'))
    note = fields.Text()
    category_id = fields.Many2one('account.asset.category', string='Catégorie', required=True, change_default=True, readonly=True, states={'draft': [('readonly', False)]})
    ...
    method = fields.Selection([('linear', 'Linear'), ('degressive', 'Degressive')], string='Méthode de calcul', required=True, readonly=True, states={'draft': [('readonly', False)]}, default='linear',
        help="Choose the method to use to compute the amount of depreciation lines.\n  * Linear: Calculated on basis of: Gross Value / Number of Depreciations\n"
            "  * Degressive: Calculated on basis of: Residual Value * Degressive Factor")
    method_number = fields.Integer(string='Nombre d\'amortissements', readonly=True, states={'draft': [('readonly', False)]}, default=5, help="The number of depreciations needed to depreciate your asset")
    ...
    method_end = fields.Date(string='Date fin', readonly=True, states={'draft': [('readonly', False)]})
    method_progress_factor = fields.Float(string='Facteur dégressif', readonly=True, default=0.3, states={'draft': [('readonly', False)]})
    value_residual = fields.Float(compute='_amount_residual', method=True, digits=0, string='Valeur résiduelle')
    method_time = fields.Selection([('number', 'Number of Entries'), ('end', 'Ending Date')], string='Méthode de temps', required=True, readonly=True, default='number', states={'draft': [('readonly', False)]},
        help="Choose the method to use to compute the dates and number of entries.\n"
             "  * Number of Entries: Fix the number of entries and the time between 2 depreciations.\n"
             "  * Ending Date: Choose the time between 2 depreciations and the date the depreciations won't go beyond.")
    prorata = fields.Boolean(string='Prorata Temporis', readonly=True, default=True,states={'draft': [('readonly', False)]},
    ...
    invoice_id = fields.Many2one('account.move', string='Facture', states={'draft': [('readonly', False)]}, copy=False)
    ...
    related_vendor_invoice_id = fields.Many2one(string='Facture Fournisseur', comodel_name='account.move', domain=[('move_type', '=', 'in_invoice')])
    
    @api.onchange('related_vendor_invoice_id')
    def onchange_related_vendor_invoice_id(self):
        for rec in self:
            if rec.related_vendor_invoice_id:
                for move in rec.related_vendor_invoice_id:
                    rec.name = move.name
                    rec.date = move.date
                    rec.invoice_date = move.invoice_date
                    for line in move.invoice_line_ids:
                        rec.value += line.price_subtotal 
    ...
    def validate(self):
        ...
        for asset in self:
            ...
            dummy, tracking_value_ids = asset._mail_track(tracked_fields, dict.fromkeys(fields)) #i modify here
...
class AccountAssetDepreciationLine(models.Model):
    ...
    asset_id = fields.Many2one('account.asset.asset', string='Immobilisation', required=True, ondelete='cascade')
    parent_state = fields.Selection(related='asset_id.state', string='Statue')
    ...
    move_id = fields.Many2one('account.move', string='Saisie d\'amortissement')
    move_check = fields.Boolean(compute='_get_move_check', string='Lié', track_visibility='always', store=True)
    move_posted_check = fields.Boolean(compute='_get_move_posted_check', string='Posté', track_visibility='always', store=True)
