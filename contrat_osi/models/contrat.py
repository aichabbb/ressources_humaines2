...
class categorie(models.Model):
    ...
    def function_duree_essai(self):
        ...
        for r in self:
            ...
            if r.bool_période2 == True:

                if r.contract_type_id.name == "contrat à durée indéterminée (CDI)":
                    r.int_r = r.période.Duree + 3
                    if r.duree_essai > r.période.Duree + 3:
                        raise ValidationError(_('ne peux pas dépasse priode essai  initiale.'))
                        r.int_r = r.duree_essai
                        r.bool_test = False
                    if r.duree_preavie > r.preavis.Duree + 3:
                        r.int_r = r.preavis.Duree
                        raise ValidationError(_('ne peux pas dépasse priode essai  initiale.'))

                        r.bool_test = False
            elif r.bool_période2 == False:
                if r.contract_type_id.name == "contrat à durée indéterminée (CDI)":
                    r.int_r = r.période.Duree * 2
                    if r.duree_essai > r.période.Duree * 2:
                        raise ValidationError(_('ne peux pas dépasse priode essai  initiale.'))
                        r.bool_test = False
                    if r.duree_preavie > r.preavis.Duree * 2:
                        raise ValidationError(_('ne peux pas dépasse priode essai  initiale.'))
