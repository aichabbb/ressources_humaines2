...
class HrCotisation(models.Model):
    ...
    type = fields.Selection([('amount', 'Montant'), ('taux', 'Taux')], default='taux')
    tauxsalarial = fields.Float(string="Taux Salarial", help=u'Montant ou taux salarial selon le type selectionné')
    tauxpatronal = fields.Float(string="Taux Patronal", help=u'Montant ou taux salarial selon le type selectionné')
...
class HrRubriqueSettings(models.TransientModel):
    ...
    def onchange_company_id(self):
        if self.company_id:
            ...
            count_rubrique = self.sudo().env['hr.payroll_ma.rubrique'].search_count([('company_id', '=', company.id)])
            count_cotisation = self.sudo().env['hr.payroll_ma.cotisation'].search_count([('company_id', '=', company.id)])
            count_parametre = self.sudo().env['hr.payroll_ma.parametres'].search_count([('company_id', '=', company.id)])
            count_journal = self.sudo().env['account.journal'].search_count([('company_id', '=', company.id), ('code', '=', 'Paie')])
            if count_rubrique > 0:
            ...
            print('gggggggggg', self.has_journal,self.has_parametre ,self.has_cotisation)
   ...
   # @api.multi
    ...
    def create_rubrique(self):
        ...
        while i < 25:
            ...
            if rubrique_id:
                ...
                rubrique = self.env['hr.payroll_ma.rubrique'].browse(rubrique_id)
                vals = self.prepare_data_rubrique(rubrique, i)
                if vals:
                    self.env['hr.payroll_ma.rubrique'].create(vals)
                i += 1
        hsup_25 = self.env['hr.payroll_ma.rubrique'].browse(self.env.ref('hr_payroll_ma.hsup_25').id)
        hsup_50 = self.env['hr.payroll_ma.rubrique'].browse(self.env.ref('hr_payroll_ma.hsup_50').id)
        hsup_100 = self.env['hr.payroll_ma.rubrique'].browse(self.env.ref('hr_payroll_ma.hsup_100').id)
        jrs_chomes_payes = self.env['hr.payroll_ma.rubrique'].browse(self.env.ref('hr_payroll_ma.jrs_chomes_payes').id)
        jrs_conges_payes = self.env['hr.payroll_ma.rubrique'].browse(self.env.ref('hr_payroll_ma.jrs_conges_payes').id)
    ...
    def create_cotisation(self):
        ...
        while i <= 7:
            ...
            if cotisation_id:
                cotisation = self.env['hr.payroll_ma.cotisation'].browse(cotisation_id.id)
    ...
    def create_parametre(self):
        ...
        if parametre_id:
            parametre = self.env['hr.payroll_ma.parametres'].browse(parametre_id.id)
            vals = {
    ...
    def create_journal(self):
        ...
        jouranl_id = self.env.ref('hr_payroll_ma.account_salary_journal')
        if jouranl_id:
            journal = self.env['account.journal'].browse(jouranl_id.id)
            vals = {
                'name': journal.name,
                'code': journal.code,
                'type': journal.type,
    ...
    def generate_data_paie(self):
        self.sudo().create_rubrique()
        self.sudo().create_cotisation()
        self.sudo().create_parametre()
        self.sudo().create_journal()
        return {
