...
class HrEmployee(models.Model):
    ...
    # anciennete
    annees_anciennete = fields.Float(string=u'Ancienneté', compute='calc_seniority',store=True)
    taux_anciennete = fields.Float(string=u'Taux ancienneté(%)', compute='calc_seniority',store=True)
    matricule_mutuelle = fields.Char('Matricule mutuelle')
   ...
   # @api.one
    ...
    def _check_param_logement_social(self):
        if self.type_logement == 'social':
            dictionnaire = self.get_parametres()
...
class HrContract(models.Model):
    ...
    hour_salary = fields.Float(u'Taux')
    monthly_hour_number = fields.Float(u'Nombre Heures par mois')
    ir = fields.Boolean(u'IR?', default=True)
    cotisation = fields.Many2one('hr.payroll_ma.cotisation.type', string=u'Type cotisation', required=False)
    ...
    salary_type = fields.Selection([('h', 'Horaire'),
                                    ('j', 'Journalier'),
                                    ('m', 'Mensuel')], required=True, default='m', string='Type')
